...
object ASMParser extends RegexParsers {
  ...
  private def twoAryInstruction: Parser[Instruction] = instructionCode ~ (immediate | address) ~ argSeparator ~ register  ^^ {case inst ~ arg1 ~ _ ~ arg2 => Instruction(inst, arg1, arg2)}
  private def oneAryInstruction: Parser[Instruction] = instructionCode ~ (immediate | address)  ^^ {case inst ~ arg1 => Instruction(inst, arg1)}
  ...
  private def indirectXInstruction: Parser[IndirectX] =
   instructionCode ~  "(" ~ (immediate | address)  ~ argSeparator ~ register ~ ")" ^^ {case inst ~ _ ~ arg1 ~ _ ~ arg2 ~ _  => IndirectX(Instruction(inst, arg1, arg2))} // Ex: STA ($15,X)

  private def indirectYInstruction: Parser[IndirectY] =
    instructionCode ~ "(" ~ (immediate | address) ~  ")" ~ argSeparator ~ register ^^ {case inst ~ _ ~ arg1 ~ _ ~ _ ~ arg2 => IndirectY(Instruction(inst, arg1, arg2))} // Ex: STA ($15), Y

  private def instruction: Parser[Instruction] = twoAryInstruction | oneAryInstruction | zeroAryInstruction

  private def directive: Parser[ASTNode] = indirectXInstruction | indirectYInstruction | instruction | labelDefinition | any
